...
var cklActionFlags=[
	{typ:"Record ON"},
	{typ:"Value from Cache",class:"jo-parsec"}, // secondary Parameter
	{typ:"Check Alarms"},
];

var cklRecFlags=[
	{typ:"Record ON"},
	{typ:"Ring Memory (recommended)",class:"jo-parsec"},
];
...
function modal_show(modalFormName){
	...
	case "modalAlert": msgActiveFlag|=1; break;
	case "modalAddMAC": msgActiveFlag|=2; break;
	case "modalEditParameter": msgActiveFlag|=4; break;
	case "modalEditDevice": msgActiveFlag|=8; break;
	case "modalEditUser": msgActiveFlag|=16; break;
	case "modalEditInfo": msgActiveFlag|=32; break;
	case "modalRemoveMAC": msgActiveFlag|=64; break;
	case "modalClearDevice": msgActiveFlag|=128; break;
	case "modalShowWEA": msgActiveFlag|=256; break;
	default: alert("Form? "+modalFormName);
	}
...
function removeSubmit(event){	
	...
	user_poll( {cmd:"removeDevice", oldmac:mac} );
...
function addSubmit(event){	// Submit Form AddDevice
	...
	event.preventDefault();
	lastSeenTimestamp=0;	// Get ALL
	if(!addDevTyp) user_poll( {cmd:"addDevice", newmac:mac, newtok:owTok} );
	else user_poll( {cmd:"addGuestDevice", newmac:mac, newtok:owTok} );
...
function deltaToTimeString(delta){ // Helper Func
	var h, lstxt="";
	if(delta>=86400){
		h=Math.floor(delta/86400); delta-=86400*h;
		lstxt+=h+"d";
	}
	h=Math.floor(delta/3600); delta-=3600*h;
	if(h<10) lstxt+="0";
	lstxt+=h+"h";
	h=Math.floor(delta/60); delta-=60*h;
	if(h<10) lstxt+="0";
	lstxt+=h+"m";
	if(delta<10) lstxt+="0";
	lstxt+=delta+"s";
	...
	$.post("w_php/w_main.php", jcmd, function( data ) {
				...
				if(adev.units !== null && adev.units.includes(":Lat") && adev.units.includes(":Lng")){
					if(isguest){ // Access with token
						...
						gpsinfo="<br><a class='jo-mac' href='gps_view.html?s="+adev.mac+"&lim=1000"+
							"' target='_blank'><b><i class='fas fa-map-marker-alt w3-text-green'></i>&nbsp; GPS Position View</b></a>";
					}
				}
		...
		//console.log(adev.mac); // <- DEBUG
			...
			if(nlc){ // New Lines: Show Info
				...
				if(adev.warnings_cnt){ cont+=", Warnings:&nbsp;"+adev.warnings_cnt; ccol="w3-yellow";}
				if(adev.err_cnt){ cont+=", Errors:&nbsp;"+adev.err_cnt; ccol="w3-red";}
				if(adev.alarms_cnt){ cont+=", Alarms:&nbsp;"+adev.alarms_cnt; ccol="w3-purple";}
				log_info(cont, ccol);
			}
	...
	if(adev.vals!==null){
		...
		for(var ii=0;ii<vals.length;ii++){
			...
			if(!isNaN(kvn) && kvn>=0 &&  kvn<200 && kv.length==2 && kv[1].length>=1 && units!=undefined){
				for(var i=0;i<units.length;i++) if(parseInt(units[i])==kvn){
...
function removeWarnings(idx){
	user_poll( {cmd:"removeWarnings", mac: deviceWList[idx].mac} );
}
function removeErrors(idx){
	user_poll( {cmd:"removeErrors", mac: deviceWList[idx].mac} );
}
function removeAlarms(idx){
	user_poll( {cmd:"removeAlarms", mac: deviceWList[idx].mac} );
}
...
function editDeviceInfo(idx){
	...
	user_poll( {cmd:"getInfo", mac:editMAC } ); // device_info.dat
...
function infoGetWEA(lstart,lanz){
	...
	logLanz=lanz;
	logRaw=[];
	user_poll( {cmd:"getWEA", mac:editMAC, pos0:lstart, anz:lanz} );
}
...
function clearDeviceSubmit(){
	...
	user_poll( {cmd:"clearDevice", mac:clearMAC } ); 
}
...
function infoPosUpdateSelect(){ // Update Type of Pos.Selection
	...
	user_poll( {cmd:"setPosUpdate", mac:editMAC, posflags:nrl } ); 
}
...
function infoEstimatePos(){
	...
	user_poll( {cmd:"getPos", mac:editMAC, cell:cellObj } );	
}
...
function edInfoFill(){
	...
	case 2: reas="AUTOMATIC "; break;
	case 3: reas="MANUAL "; break;
	default: reas="UNKNOWN("+infoObj.reason+") ";
	}
	...
	if(cellObj.dbm>=-1) {
		...
		if(cellObj.dbm>=-70) {qual="Top"; squal+="light-green'>";}
		else if(cellObj.dbm>=-85) {qual="Good"; squal+="yellow'>";}
		else if(cellObj.dbm>=-100) {qual="Fair"; squal+="orange'>";}
		else  {qual="Poor"; squal+="red'>";} // Poor or none
...
function infoGetLog(id,lstart,lanz){
	...
	user_poll( {cmd:"getLog", mac:editMAC, typ:id, pos0:lstart, anz:lanz} );
}
...
function infoSavePos(){
	if(getPosCoords()==true){
		...
		user_poll( {cmd:"savePos", mac:editMAC, newpos:saveObj} );
	}
}
function infoClearPos(){
		...
		user_poll( {cmd:"clearPos", mac:editMAC} );
}
//--- Device Infos END ----
...
function editDeviceParameter(idx){	// Show
	...
	user_poll( {cmd:"getParam", mac:editMAC } );
...
function edParamIdxFind(aline, idx){ // Find Index of Current Param
	console.log("A["+idx+"]='"+aline+"' ");	console.log(typeof aline);
...
function editDeviceDetails(idx){	// Show
	user_poll( {cmd:"getDevice", mac: deviceWList[idx].mac } );
...
function edMailTest0(){
	...
	if(checkSaved()!=true || newcontact0.length<5){	// Might later be something else than Mail
		...
		*/
		user_poll( {cmd:"testContact", contNo:0, mac:editDeviceData.mac, xcont:mcont /*, mail:editDeviceData.email0*/ } );
	}
	event.preventDefault();
}
function edResetCond0(){
	...
	if(checkSaved()!=true){	// Might later be something else than Mail
		...
		user_poll( {cmd:"cntReset", contNo:0, mac:editDeviceData.mac} );
	}
}
...
function editUserSubmit(){
	...
	if(newname!=userData.name){ // Send what is changed
		user_poll( {cmd:"changeUser", new_name:newname} );
		document.getElementById("modalSpinner").style.display="block";
		spinnerVisible=true;
	} // Name unchanged;
}
